    ...
    public function printFiles($thread, $canManage)
    {
        ...
        foreach($thread->files as $file)
        {
            ...
            if($file->isImage)
            {
                ...
                $imagesHtml .= "<li class='file-image file-{$file->extension}'>" . html::a(helper::createLink('file', 'download', "fileID=$file->id&mose=left"), html::image(helper::createLink('file', 'read', "fileID={$file->id}&type=fullURL", '', "{$file->extension}")), "target='_blank' data-toggle='lightbox'");
