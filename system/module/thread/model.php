    ...
    public function printFiles($thread, $canManage)
    {
        ...
        foreach($thread->files as $file)
        {
            ...
            if($file->isImage)
            {
                ...
                $imagesHtml .= "<li class='file-image file-{$file->extension}'>" . html::a(helper::createLink('file', 'download', "fileID=$file->id&mose=left"), html::image("/file.php?pathname={$file->pathname}&objectTyp={$file->objectType}&imageSize=&extension={$file->extension}"), "target='_blank' data-toggle='lightbox'");
