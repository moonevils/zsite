...
<div class='panel'>
  ...
  <div id='imageView' class='panel-body'>
    ...
    <?php foreach($files as $file):?>
        ...
        $imagePath = "{$config->webRoot}file.php?f={$file->pathname}&o={$file->objectType}&t={$file->extension}&v={$this->config->site->lastUpload}";
        if($file->isImage)
        {
      ...
      <tbody>
        ...
        <tr class='text-center text-middle'>
          ...
          <td>
            ...
            if($file->isImage)
            {
                echo html::a(inlink('download', "id=$file->id"), html::image("{$config->webRoot}file.php?f={$file->pathname}&o={$file->objectType}&s=smallURL&t={$file->extension}&v={$this->config->site->lastUpload}", "class='image-small' title='{$file->title}'"), "data-toggle='lightbox' target='_blank'");
            }
            else
            {
          ...
          <td class='text-left'><?php echo "{$config->webRoot}file.php?f={$file->pathname}&o={$file->objectType}&t={$file->extension}&v={$this->config->site->lastUpload}";?></td>
