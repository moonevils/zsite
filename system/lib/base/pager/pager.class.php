...
EOT;
     ...
     * @return string
     */
    public function createUrl($pageID = '')
    {
        if(!$pageID) $pageID = $this->params['pageID'];
        global $config;
        if($config->requestType != 'GET' && method_exists('uri', 'create' . $this->moduleName . $this->methodName))
        {
            ...
            if($pageID == 1) return preg_replace('/\/p\d+\./', '.', $link);

            if(preg_match('/\/p\d+/', $link)) return preg_replace('/\/p\d+\./', '/p' . $pageID . '.', $link);

            if($config->requestType == 'PATH_INFO2') $link = str_replace('index.php/', 'index_php/', $link);
            if(strpos($link, '.') !== false) $link = str_replace('.', "/p{$pageID}.", $link);
    ...
    {
        return html::a($this->createUrl(), $title);
    }
}
