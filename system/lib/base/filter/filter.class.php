...
class baseValidater
{
    ...
    {
        return preg_match("/^(1[3-5,7,8]{1}[0-9]{9})|(199){1}[0-9]{8}$/", $var);
    }
    ...
    public static function filterParam($var, $type)
    {
        ...
        if(isset($filter->{$moduleName}->{$methodName}->{$type}) and $filter->{$moduleName}->{$methodName}->{$type} === 'all') return $var;
...
class baseFixer
{
    ...
    public function stripTags($fieldName, $allowedTags = '')
    {
        ...
        if($usePurifier)
        {
            ...
            $purifierConfig->set('HTML.SafeObject', 1);
        ...
        foreach($fields as $fieldName)
        {
            ...
            if(!in_array($fieldName, $this->stripedFields))
            {
                ...
                {
                    /*
                     * purifier会把&nbsp;替换空格，kindeditor在会把行首的空格去掉。
