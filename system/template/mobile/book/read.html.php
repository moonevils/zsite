      ...
      <div class='panel-section article' id='book' data-id='{!echo $article->id}'>
        ...
        <div class='panel-footer'>
          ...
          <?php extract($prevAndNext)}
            {if($prev)}
            {!echo html::a(inlink('read', "articleID=$prev->id", "book={$book->alias}&node={$prev->alias}") . ($control->get->fullScreen ? "?fullScreen={$control->get->fullScreen}" : ''), "<i class='icon-arrow-left'></i> " . $prev->title, "class='btn block text-left default'")}
            {else}
            <a href='###' class='btn block text-left default disabled'><i class='icon-arrow-left'></i> {!print($lang->book->none)}</a>
            {/if}
            {if($next)}
            {!echo html::a(inlink('read', "articleID=$next->id", "book={$book->alias}&node={$next->alias}") . ($control->get->fullScreen ? "?fullScreen={$control->get->fullScreen}" : ''), "<i class='icon-arrow-right'></i> " . $next->title, "class='btn block text-left default'")}
            {else}
            <a href='###' class='btn block text-left default disabled'>{!print($lang->book->none)}<i class='icon-arrow-right'></i></a>
            {/if}
            {!echo html::a(inlink('browse', "bookID={$parent->id}", "book={$book->alias}&title={$parent->alias}") . ($control->get->fullScreen ? "?fullScreen={$control->get->fullScreen}" : ''), "<i class='icon-list-ul'></i> " . $lang->book->chapter, "class='btn block text-left default'")}
