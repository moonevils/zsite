...
<div class='block {!echo $blockClass}' id='block{!echo $block->id?>'>
  ...
  {/if}
      ...
      <div class='item{if($index === 0) echo ' active'}'{!echo $url . ' ' . $target}>
      {!print(html::image($slide->image))}
      {else}
      <div class='item{if($index === 0) echo ' active'}'{!echo $url . ' ' . $target} style='{!echo 'background-color: ' . $slide->backgroundColor . '; height: ' . $height . 'px'}'>
      {/if}
    ...
    {if($slideStyle == 'carousel')}
      ...
      {if(count($slides) > 1)}
        <a class='left carousel-model' href='#{!echo $slideId}' data-slide='prev'> <i class='icon icon-chevron-left'></i> </a>
        <a class='right carousel-model' href='#{!echo $slideId}' data-slide='next'> <i class='icon icon-chevron-right'></i> </a>
      {/if}
    {/if}
  </div>
</div>
