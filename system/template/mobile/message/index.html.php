  ...
  <div id='commentsListWrapper'><div id='commentsList'> <?php // Double div for ajax load. ?>
    ...
    <div class='cards condensed bordered'>
      ...
      <div class='card comment'>
        <div class='card-heading'>
          <span class='text-special name'>{!echo $message->from?></span> &nbsp; <small class='text-muted time'>{!echo formatTime($message->date, 'Y/m/d H:m')}</small>
          <div class='actions'>
            {!echo html::a($control->createLink('message', 'reply', "commentID=$message->id"), $lang->message->reply, "data-toggle='modal' data-type='ajax' data-icon='reply' data-title='{$lang->message->reply}'")}
          </div>
        </div>
        <div class='card-content'>{!echo nl2br($message->content)}</div>
    ...
    <div class='panel-body'>
      ...
      <div class='alert text-center bg-primary-pale text-info'>
        <p><i class='icon-comments-alt icon-s3'></i></p><strong>0 {!echo $lang->message->common?></strong>
      </div>
    </div>
    {/if}
  </div></div>
  ...
  <div class='modal-dialog'>
    <div class='modal-content'>
      ...
      <div class='modal-body'>
        <form method='post' id='commentForm' action="{!echo $control->createLink('message', 'post', 'type=message')}">
...
{*/php*}
          ...
          <div class='form-group'>
            <label><small class='text-important'>{!echo $lang->message->contactHidden}</small></label>
