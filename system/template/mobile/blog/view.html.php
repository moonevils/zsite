...
<div class='appheader'>
  <div class='heading'>
    ...
    <div class='caption text-muted'>
      <small><i class='icon-time icon-large'></i> {!echo formatTime($article->addedDate)}</small> &nbsp;&nbsp;
      <small><i class='icon-user icon-large'></i> {!echo $article->author}</small> &nbsp;&nbsp;
      <small><i class='icon-eye-open'></i> {!echo $config->viewsPlaceholder}</small> &nbsp;&nbsp;
      {if($article->source != 'original' and $article->copyURL != '')}
      <small>{!echo $article->source ? $lang->article->lblSource : $lang->article->sourceList[$article->source] . $lang->colon}{$article->copyURL ? print(html::a($article->copyURL, $article->copySite, "target='_blank'")) : print($article->copySite)}</small>
      {else}
      <small class='text-success bg-success-pale'>{!echo $lang->article->sourceList[$article->source]}</small>
      {/if}
    </div>
  </div>
</div>

<div class='panel-section article' id="blog{!echo $article->id?>" data-ve='blog'>
  {if($article->summary)}
  <section class='abstract hide bg-gray-pale small with-padding'><strong>{!echo $lang->article->summary}</strong>{!echo $lang->colon . $article->summary}</section>
  {/if}
  ...
  <div class='panel-footer'>
    ...
    <ul class='pager pager-justify'>
      {if($prev)}
      <li class='previous'>{!echo html::a(inlink('view', "id=$prev->id", "category={$category->alias}&name={$prev->alias}"), '<i class="icon-arrow-left"></i> ' . $lang->article->previous, "title='{$prev->title}'")}</li>
      {else}
      <li class='previous disabled'><a href='###'><i class='icon-arrow-left'></i> {!print($lang->article->none)}</a></li>
      {/if}
      {if($next)}
      <li class='next'>{!echo html::a(inlink('view', "id=$next->id", "category={$category->alias}&name={$next->alias}"), $lang->article->next . ' <i class="icon-arrow-right"></i>', "title='{$next->title}'")}</li>
      {else}
