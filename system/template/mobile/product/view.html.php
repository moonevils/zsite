...
<?php if(!empty($product->image->list)):?>
  ...
  <div id='productSlide' class='carousel slide' data-ride='carousel'>
    ...
    <?php foreach($product->image->list as $image):?>
      <div class='item<?php if($imgIndex === 0) echo ' active';?>'><?php echo html::image($this->loadModel('file')->printFileURL($product->image->primary->pathname, $product->image->primary->extension, 'product', 'middleURL'), "title='{$title}' alt='{$product->name}'");?></div>
  ...
  <?php else:?>
  <?php echo html::image($this->loadModel('file')->printFileURL($product->image->primary->pathname, $product->image->primary->extension, 'product', 'largeURL'), "title='{$title}' alt='{$product->name}'");?>
  <?php endif;?>
<?php endif;?>
  <div class='heading'>
