...
{$template       = $control->config->template->{{$control->app->clientDevice}}->name ? $control->config->template->{{$control->app->clientDevice}}->name : 'default'}
{$theme          = $control->config->template->{{$control->app->clientDevice}}->theme ? $control->config->template->{{$control->app->clientDevice}}->theme : 'default'}
...
<head>
  ...
  {if(isset($mobileURL))}
  <link rel="alternate" media="only screen and (max-width: 640px)" href="{!echo $sysURL . ltrim($mobileURL, '/')}">
  {/if}
  {if(isset($sourceURL))}
  <link rel="canonical" href="{!echo $sysURL . ltrim($sourceURL, '/')}" >
  {/if}
 ...
 {$customCssFile = $control->loadModel('ui')->getCustomCssFile($config->template->{{$control->app->clientDevice}}->name, $config->template->{{$control->app->clientDevice}}->theme)}
 {if(file_exists($customCssFile))} {!css::import($control->ui->getThemeCssUrl($template, $theme))} {/if} 

  {!js::exportConfigVars()}
  {!js::set('theme', array('template' => $config->template->{{$control->app->clientDevice}}->name, 'theme' => $config->template->{{$control->app->clientDevice}}->theme, 'device' => $control->app->clientDevice))}
  {if($config->debug)}
  ...
  {!echo html::rss($control->createLink('rss', 'index', '', '', 'xml'), $config->site->name)}
  {!js::set('lang', $lang->js)}
...
{if(empty($sina->verification) && !empty($sina->widget))} {js::import('http://tjs.sjs.sinajs.cn/open/api/js/wb.js')}{/if}
<!--[if lt IE 9]>
{if($config->debug)}
      ...
      <div class="wrapper">
        ...
        {if($logo)}
          ...
          <div id='siteLogo' data-ve='logo'>
            {!echo html::a(helper::createLink('index'), html::image($control->loadModel('file')->printFileURL($logo->pathname, $logo->extension), "class='logo' alt='{{$control->config->company->name}}' title='{{$control->config->company->name}}'"));}
          </div>
          {else}
          <div id='siteName' data-ve='logo'><h2>{!echo html::a(helper::createLink('index'), $control->config->site->name)}</h2></div>
        {/if}
      </div>
    </div>
  ...
  <nav id="blogNav" class="navbar navbar-default" data-ve='navbar' data-type='desktop_blog'>
    <div class='wrapper'>
      ...
      <ul class="nav navbar-nav navbar-right">
        <li>{!echo html::a(helper::createLink('index'), '<i class="icon-home icon-large"></i> ' . $lang->siteHome)}</li>
      </ul>
      {/if}
    </div>
  </nav>
