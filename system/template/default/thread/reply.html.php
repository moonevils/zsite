...
<div id = "{!echo $reply->id}" class="panel panel thread reply {!echo $floor%2!=0?'striped':''}">
  <div class='panel-heading'>
    <div class='panel-actions'>
      <strong>{if($floor > 2) echo '#' . $floor}</strong>
  ...
  <div class='thread-foot'>
    ...
    <span class="thread-more-actions">
      ...
      {/if}
      <?php
      if($control->thread->canManage($board->id)) echo html::a($control->createLink('reply', 'delete', "replyID=$reply->id"), '<i class="icon-trash"></i> ' . $lang->delete, "class='deleter'");
      ?>
      {if($control->thread->canManage($board->id, $reply->author)) echo html::a($control->createLink('reply', 'edit',   "replyID=$reply->id"), '<i class="icon-pencil"></i> ' . $lang->edit)}
    </span>
