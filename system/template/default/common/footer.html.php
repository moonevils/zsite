  ...
  <footer id='footer' class='clearfix'>
    <div class='wrapper'>
      ...
      <span id='copyright'>
       ...
       &copy; {$copyright} {function="date('Y')"} {$company} &nbsp;&nbsp;
      </span>
      <span id='icpInfo'>
        {if(!empty($config->site->icpLink) and !empty($config->site->icpSN))}
          {function="html::a(strpos($config->site->icpLink, 'http://') !== false ? $config->site->icpLink : 'http://' . $config->site->icpLink, $config->site->icpSN, "target='_blank'")"}
        {/if}
        ...
        {if(!empty($config->site->policeLink) and !empty($config->site->policeSN))}
          {function="html::a(strpos($config->site->policeLink, 'http://') !== false ? $config->site->policeLink : 'http://' . $config->site->policeLink, html::image($webRoot . 'theme/default/default/images/main/police.png'), "target='_blank'")"}
        {/if}
      </span>
      <div id='powerby'>
          {function="commonModel::printPowerdBy()"}
      </div>
      {if($config->site->execInfo == 'show')} {$config->execPlaceholder} {/if}
    </div>
  </footer>
  {if($config->debug)} {function="js::import($jsRoot . 'jquery/form/min.js')"} {/if}
  {if(isset($pageJS))} {function="js::execute($pageJS)"} {/if}
