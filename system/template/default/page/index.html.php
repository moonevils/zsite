...
<div class='row'>
  <div class='col-md-9'>
    <div class='list list-condensed'>
      <header><h2>{!echo $control->lang->page->list}</h2></header>
        ...
        <div class='item' id='page{!echo $page->id}' data-ve='page'>
          ...
          <div class='item-content'>
            ...
            <div class='media pull-right'>
              ...
              echo html::a($url, html::image("{$config->webRoot}file.php?pathname={$page->image->primary->pathname}&objectType=page&imageSize=smallURL&extension={$page->image->primary->extension}", "title='{$title}' class='thumbnail'" ));
              ?>
            </div>
            {/if}
            <div class='text text-muted'>{!echo helper::substr($page->summary, 120, '...')}</div>
          </div>
          <div class='item-footer text-muted'>
            <span><i class='icon-eye-open'></i> {!echo $page->views}</span> &nbsp;
            <span><i class='icon-time'></i> {!echo substr($page->addedDate, 0, 10)}</span>
          </div>
        </div>
        {/foreach}
      </section>
