    ...
    <div class='content'>
      ...
      <div class='actions'>
        {if($refererBeforeDeny)}
            {!html::a(helper::safe64Decode($refererBeforeDeny), $lang->user->goback, "class='btn btn-primary'")}
        {/if}
        {!html::a($control->createLink($config->default->module), $lang->index->common, "class='btn'")}
        {if(commonModel::isAvailable($module) and $control->app->user->account == 'guest')}
            {!html::a($control->createLink('user', 'register'), $lang->user->register->common, "class='btn btn-link'")}
        {/if}
        {if(commonModel::isAvailable($module))}
