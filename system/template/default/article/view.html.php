...
<div class='row' id='columns' data-page='article_view'>
    ...
    <div class='article' id='article' data-id='{$article->id}'>
      <header>
        ...
        <dl class='dl-inline'>
          ...
          <dd>{!echo $article->copyURL ? print(html::a($article->copyURL, $article->copySite, "target='_blank'")) : print($article->copySite)}</dd>
          {else}
          ...
          <dd class='pull-right'>
            {if(!empty($control->config->oauth->sina))}
              ...
              {if(isset($sina->widget))}
                {!echo "<div class='sina-widget'>" . $sina->widget . '</div>'}
              {/if}
            {/if}
        ...
        {if($article->summary)}
        <section class='abstract'><strong>{$lang->article->summary}</strong>{$lang->colon . $article->summary}</section>
        {/if}
      </header>
      ...
      <footer>
        ...
        <ul class='pager pager-justify'>
          {if($prev)}
          <li class='previous' title='{$prev->title}'>{!echo html::a(inlink('view', "id=$prev->id", "category={{$category->alias}}&name={{$prev->alias}}"), '<i class="icon-arrow-left"></i> <span>' . $prev->title . '</span>')}</li>
          {else}
          <li class='preious disabled'><a href='###'><i class='icon-arrow-left'></i> {$lang->article->none}</a></li>
          {/if}
          {if($next)}
          <li class='next' title='{$next->title}'>{!echo html::a(inlink('view', "id=$next->id", "category={{$category->alias}}&name={{$next->alias}}"), '<span>' . $next->title . '</span> <i class="icon-arrow-right"></i>')}</li>
          {else}
          <li class='next disabled'><a href='###'> {$lang->article->none}<i class='icon-arrow-right'></i></a></li>
          {/if}
        </ul>
      </footer>
    </div>
  ...
  {if(!empty($layouts['article_view']['side']) and !(empty($sideFloat) || $sideFloat === 'hidden'))}
  <div class='col-md-{$sideGrid} col-side'><side class='page-side blocks' data-region='article_view-side'>{$control->block->printRegion($layouts, 'article_view', 'side')}</side></div>
  {/if}
</div>
