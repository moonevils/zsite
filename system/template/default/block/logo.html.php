...
<div class='site-logo' data-ve='block' data-id='<?php echo $block->id; ?>'>
  <?php echo html::a(helper::createLink('index'), html::image("{$this->config->webRoot}file.php?f={$logo->pathname}&t={$logo->extension}&v={$this->config->site->lastUpload}", "class='logo' alt='{$this->config->company->name}' title='{$this->config->company->name}'"));?></div>
</div>
<?php else: ?>
