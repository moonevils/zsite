...
<div class='site-logo' data-ve='block' data-id='{!echo $block->id}'>
  {!html::a(helper::createLink('index'), html::image($model->loadModel('file')->printFileURL($logo->pathname, $logo->extension), "class='logo' alt='{{$lang->company->name}}' title='{{$lang->company->name}}'"))}</div>
</div>
{else}
