...
<header id='header' class='{if($setting->bottom)} {!echo 'without-navbar'} {/if}'>
  ...
  <div id='headTitle' class='{if($setting->middle->center == 'nav')} {!echo 'with-navbar'} {/if} {if($setting->middle->center == 'slogan')} {!echo ' with-slogan'} {/if} '>
    <div class='row'>
      <div id='siteTitle'>
        {if($logo)}
        <div id='siteLogo' data-ve='logo'>{!html::a(helper::createLink('index'), html::image($model->loadModel('file')->printFileURL($logo->pathname, $logo->extension), "class='logo' alt='{{$config->company->name}}' title='{{$config->company->name}}'"))}</div>
        {else}
