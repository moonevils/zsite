...
<header id='header' class='<?php if($setting->bottom) echo 'without-navbar'; ?>'>
  ...
  <div id='headNav' class='<?php if($setting->top->left == 'slogan') echo 'with-slogan' ?><?php if($isWidthSearchBar) echo ' with-searchbar' ?>'>
    <div class='row'>
      ...
      <div class='nobr'><span><?php echo htmlspecialchars_decode($setting->topLeftContent, ENT_QUOTES);?></span></div>
      <?php endif;?>
  ...
  <div id='headTitle' class='<?php if($setting->middle->center == 'nav') echo 'with-navbar' ?><?php if($setting->middle->center == 'slogan') echo ' with-slogan' ?>'>
    <div class='row'>
      <div id='siteTitle'>
        <?php if($logo):?>
        <div id='siteLogo' data-ve='logo'><?php echo html::a(helper::createLink('index'), html::image($this->loadModel('file')->printFileURL($logo->pathname, $logo->extension), "class='logo' alt='{$this->config->company->name}' title='{$this->config->company->name}'"));?></div>
        <?php else: ?>
