...
{if($setting->top->right and strpos(',searchAndLogin,search,loginAndSearch,', ',' . $setting->top->right . ',') !== false)} {$isWidthSearchBar = true} {/if}
{if($setting->top->right == 'custom')}
  ...
  {/foreach}
{/if}
<header id='header' class='{if($setting->bottom)} without-navbar {/if} '>
  {if($setting->top->left or $setting->top->right)}
  <div id='headNav' class='{if($setting->top->left == 'slogan')} with-slogan {/if} {if($isWidthSearchBar)} with-searchbar {/if}'>
    <div class='row'>
      {if($setting->top->left == 'slogan')}
        <div id='siteSlogan' class='nobr'><span>{!echo $config->site->slogan}</span></div>
      {elseif($setting->topLeftContent)}
        <div class='nobr'><span>{!echo htmlspecialchars_decode($setting->topLeftContent, ENT_QUOTES)}</span></div>
      {/if}
  ...
  <div id='headTitle' class='{if($setting->middle->center == 'nav')} with-navbar {/if} {if($setting->middle->center == 'slogan')} with-slogan {/if}'>
    <div class='row'>
      <div id='siteTitle'>
        {if($logo)}
          <div id='siteLogo' data-ve='logo'>{!echo html::a(helper::createLink('index'), html::image($model->loadModel('file')->printFileURL($logo->pathname, $logo->extension), "class='logo' alt='{{$config->company->name}}' title='{{$config->company->name}}'"))}</div>
        {else}
          <div id='siteName' data-ve='logo'><h2>{!echo html::a(helper::createLink('index'), $config->site->name)}</h2></div>
        {/if}
        {if($setting->middle->center == 'slogan')} <div id='siteSlogan' data-ve='slogan'><span>{!echo $config->site->slogan}</span></div> {/if}
      </div>
