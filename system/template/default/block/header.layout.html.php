...
<header id='header' class='<?php if($setting->bottom) echo 'without-navbar'; ?>'>
  ...
  <div id='headTitle' class='<?php if($setting->middle->center == 'nav') echo 'with-navbar' ?><?php if($setting->middle->center == 'slogan') echo ' with-slogan' ?>'>
    <div class='row'>
      <div id='siteTitle'>
        <?php if($logo):?>
        <div id='siteLogo' data-ve='logo'><?php echo html::a(helper::createLink('index'), html::image("{$this->config->webRoot}file.php?f={$logo->pathname}&t={$logo->extension}&v={$this->config->site->lastUpload}", "class='logo' alt='{$this->config->company->name}' title='{$this->config->company->name}'"));?></div>
        <?php else: ?>
