...
<div class='article book-content' id='book' data-id='{!echo $article->id?>'>
  ...
  <section class='article-content'>
    {if(isset($content) and $article->type != 'book') echo $content}
    {if($article->type == 'book') echo $article->content}
  </section>
  <section>{$control->loadModel('file')->printFiles($article->files)}</section>
  <footer>
    {if($article->keywords)}
...
{if(commonModel::isAvailable('message')) echo "<div id='commentBox'>" . $control->fetch('message', 'comment', "objectType=book&objectID={$article->id}") . "</div>"}
<div class='blocks' data-region='book_read-bottom'>{$control->block->printRegion($layouts, 'book_read', 'bottom')}</div>
{if($control->config->book->chapter == 'left')}
  </div>
</div>
