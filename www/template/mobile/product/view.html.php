...
<?php if(!empty($product->image->list)):?>
  ...
  <div id='productSlide' class='carousel slide' data-ride='carousel'>
    ...
    <?php foreach($product->image->list as $image):?>
      <div class='item<?php if($imgIndex === 0) echo ' active';?>'><?php echo html::image("/file.php?pathname={$product->image->primary->pathname}&imageSize=middleURL&extension={$product->image->primary->extension}", "title='{$title}' alt='{$product->name}'");?></div>
  ...
  <?php else:?>
  <?php echo html::image("/file.php?pathname={$product->image->primary->pathname}&imageSize=largeURL&extension={$product->image->primary->extension}", "title='{$title}' alt='{$product->name}'");?>
  <?php endif;?>
<?php endif;?>
  <div class='heading'>
